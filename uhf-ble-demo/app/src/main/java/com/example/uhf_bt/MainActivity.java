...
public class MainActivity extends BaseActivity implements View.OnClickListener {
    ...
    private ProgressBar progressBarMasterData;
    private TextView tvMasterDataStatus;
    public BluetoothAdapter mBtAdapter = null;
    public RFIDWithUHFBLE uhf = RFIDWithUHFBLE.getInstance();
    BTStatus btStatus = new BTStatus();
    
    // Master data storage
    private List<MasterDataItem> masterDataList = new ArrayList<>();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // Fetch master data saat aplikasi dibuka
        fetchMasterData();
        ...
        if (!isScanning) {
            if (item.getItemId() == R.id.UHF_Battery) {
               ...
               startActivity(intent);
                ...
                // Logout - hapus token dan kembali ke LoginActivity
                LoginActivity.clearToken(MainActivity.this);
...
//                            if (shouldShowDisconnected())
    ...
    protected void initUI() {
        ...
        progressBarMasterData = (ProgressBar) findViewById(R.id.progressBarMasterData);
        tvMasterDataStatus = (TextView) findViewById(R.id.tvMasterDataStatus);
    ...
    private void fetchMasterData() {
        try {
            ...
            if (token == null || token.isEmpty()) {
                Log.e(TAG, "Token tidak ditemukan, skip fetch master data");
                showMasterDataStatus("Token tidak tersedia", false);
                return;
            }
            
            Log.e(TAG, "Memulai fetch master data..." + token);
            showMasterDataLoading(true);
            showMasterDataStatus("Mengambil master data...", false);
            
            // Panggil API untuk fetch master data
            Call<List<MasterDataItem>> call = ApiClient.getApiService(this).getMasterData("Bearer " + token, token);
            call.enqueue(new Callback<List<MasterDataItem>>() {
                @Override
                public void onResponse(Call<List<MasterDataItem>> call, Response<List<MasterDataItem>> response) {
                    try {
                        Log.e(TAG, "res: " + response.toString());
                        showMasterDataLoading(false);
                        
                        if (response.isSuccessful()) {
                            List<MasterDataItem> masterDataList = response.body();
                            if (masterDataList != null && !masterDataList.isEmpty()) {
                                // Master data berhasil diambil
                                Log.d(TAG, "Master data berhasil diambil: " + masterDataList.size() + " items");
                                
                                // Simpan atau proses master data sesuai kebutuhan
                                processMasterData(masterDataList);
                                
                                showMasterDataStatus("Master data dimuat (" + masterDataList.size() + " items)", true);
                                
                                // Tampilkan toast sukses (opsional, hanya untuk debug)
                                if (BuildConfig.DEBUG) {
                                    runOnUiThread(() -> {
                                        Toast.makeText(MainActivity.this, "Master data berhasil dimuat (" + masterDataList.size() + " items)", Toast.LENGTH_SHORT).show();
                                    });
                                }
                            } else {
                                Log.w(TAG, "Master data response kosong atau null");
                                handleMasterDataError("Response kosong atau tidak valid");
                            }
                        } else {
                            String errorMsg = "HTTP " + response.code() + ": " + response.message();
                            Log.e(TAG, "Error fetch master data: " + errorMsg);
                            handleMasterDataError(errorMsg);
                        }
                    } catch (Exception e) {
                        showMasterDataLoading(false);
                        Log.e(TAG, "Exception dalam onResponse: " + e.getMessage());
                        handleMasterDataError("Error processing response: " + e.getMessage());
                    }
                }
                
                @Override
                public void onFailure(Call<List<MasterDataItem>> call, Throwable t) {
                    showMasterDataLoading(false);
                    String errorMsg = t.getMessage() != null ? t.getMessage() : "Unknown network error";
                    Log.e(TAG, "Gagal fetch master data: " + errorMsg);
                    handleMasterDataError("Network error: " + errorMsg);
                }
            });
            
        } catch (Exception e) {
            showMasterDataLoading(false);
            Log.e(TAG, "Exception saat fetch master data: " + e.getMessage());
            handleMasterDataError("Initialization error: " + e.getMessage());
        }
    }
    
    /**
     * Handle error saat fetch master data
     */
    private void handleMasterDataError(String errorMessage) {
        Log.e(TAG, "Master data fetch error: " + errorMessage);
        
        showMasterDataStatus("Gagal memuat master data", false);
        
        // Hanya tampilkan error dalam mode debug
        if (BuildConfig.DEBUG) {
            runOnUiThread(() -> {
                Toast.makeText(MainActivity.this, "Gagal memuat master data: " + errorMessage, Toast.LENGTH_LONG).show();
            });
        }
        
        // TODO: Implementasi fallback atau retry mechanism jika diperlukan
        // Contoh: gunakan data cache, retry setelah delay, dll
    }
    
    /**
     * Tampilkan/sembunyikan loading indicator untuk master data
     */
    private void showMasterDataLoading(boolean show) {
        runOnUiThread(() -> {
            if (progressBarMasterData != null) {
                progressBarMasterData.setVisibility(show ? View.VISIBLE : View.GONE);
            }
        });
    }
    
    /**
     * Tampilkan status master data
     */
    private void showMasterDataStatus(String message, boolean isSuccess) {
        runOnUiThread(() -> {
            if (tvMasterDataStatus != null) {
                tvMasterDataStatus.setText(message);
                tvMasterDataStatus.setVisibility(View.VISIBLE);
                
                // Auto hide status setelah beberapa detik jika sukses
                if (isSuccess) {
                    new Handler().postDelayed(() -> {
                        if (tvMasterDataStatus != null) {
                            tvMasterDataStatus.setVisibility(View.GONE);
                        }
    ...
    /**
     * Proses master data yang telah diambil dari server
     */
    private void processMasterData(List<MasterDataItem> masterDataList) {
        if (masterDataList != null && !masterDataList.isEmpty()) {
            Log.d(TAG, "Processing " + masterDataList.size() + " master data items");
            
            // Simpan master data ke variabel instance
            this.masterDataList = masterDataList;
            
            // Log untuk debugging
            for (MasterDataItem item : masterDataList) {
                Log.d(TAG, "Master data item: " + item.getAssCode() + " - " + item.getAssDesc() + " - RFID: " + item.getAssRfidCode());
            }
        }
    }
    
    /**
     * Mendapatkan master data list untuk digunakan oleh fragment
     */
    public List<MasterDataItem> getMasterDataList() {
        return masterDataList;
    }
    
    /**
     * Mencari master data berdasarkan RFID code
     */
    public MasterDataItem findMasterDataByRfidCode(String rfidCode) {
        if (masterDataList != null && rfidCode != null) {
            for (MasterDataItem item : masterDataList) {
                if (rfidCode.equals(item.getAssRfidCode())) {
                    return item;
                }
            }
        }
